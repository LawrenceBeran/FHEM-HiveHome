...
sub _mergeElement1WithHottestTemperature($$$)
{
    ...
    if (defined($elementTRV) && $elementTRV->{temp} > $elementHeating->{temp}) {
        hhc_modifyTemperature($retElement, $elementTRV->{temp}, undef);
...
sub _reduceNumberOfElements($$) 
{
    ...
    return join(" / ", map($_->{element}, (@{$dayElements_ref})));
}
    ...
    if (defined($trvDayShedule))  {
        ...
        if (defined($heatingDayShedule)) {
            ...
            if ($heatingDayShedule ne $trvDayShedule) {
                ...
                my @heatingDayElements = hhc_extractHeatingElements($heatingDayShedule);
                my @trvDayElements = hhc_extractHeatingElements($trvDayShedule);
            ...
            if (defined($hashHiveHome->{helper}{$heatingZone})) {
                ...
                if (0 != AttrVal($heatingProduct, 'setScheduleFromTRVs', 0)) {
                    ...
                    if (defined($heatingSchedule)) {
                        ...
                        my $different = undef;
                        my $tempOffset = 0; # For this process we do not want to adjust the zone heating schedule by the temperature offsets of each TRV.
...
sub HiveHome_ParseWeekCmdString($$)
{
    ...
    for (my $day = 0;$day <= $#array && $valid_string;$day += 2) {
        if (!exists($dayHash{lc($array[$day])})) {
            ...
            $weekHash->{$dayHash{lc($array[$day])}} = hhc_trim($array[$day+1]);
        }
    }
...
sub HiveHome_Write_Product($$$$@)
{
    ...
    if (!defined($shash->{STATE}) || lc($shash->{STATE}) eq 'disconnected') {
        ...
        if ($cmd eq 'weekprofile') {
            ...
            Log(4, "HiveHome_Write_Product(${cmd}): Temperature offset: ${tempOffset}, Force Update Schedule: ${forceUpdateSchedule},  WeekProfile - ${weekString}");
